...
const LAUNCHERS_UI: Record<string, { color: string; shortName: string }> = {
  "Lunar Client": { color: "bg-[#0b1016] border-[#1e2329]", shortName: "Lunar" },
  "Feather Client": { color: "bg-[#1f1f1f] border-[#333333]", shortName: "Feather" },
  "Modrinth": { color: "bg-[#1bd96a] border-[#22c55e]", shortName: "Modrinth" },
  "CurseForge App": { color: "bg-[#f16436] border-[#f97316]", shortName: "CurseForge" },
  "Prism Launcher": { color: "bg-[#25252b] border-[#3f3f46]", shortName: "Prism Launcher" },
  "GDLauncher": { color: "bg-[#0f172a] border-[#1e293b]", shortName: "GDLauncher" },
  "ATLauncher": { color: "bg-[#18181b] border-[#27272a]", shortName: "ATLauncher" },
...
export function MigrationModal({ onClose, onRefresh }: MigrationModalProps) {
  ...
  const groupedProfiles = detectedProfiles.reduce((acc, profile) => {
  ...
  if (availableLaunchers.length === 0) {
    return (
      <div className="p-8 text-center">
        <Box className="w-12 h-12 text-[#8888a0] mx-auto mb-4" />
        <h3 className="text-white font-bold mb-2">Aucun launcher détecté</h3>
        <p className="text-[#8888a0] text-sm">
          Installez des launchers comme Prism, Lunar ou Modrinth pour utiliser cette fonctionnalité.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="p-6">
        ...
        <h3 className="text-xl text-white font-bold mb-6">
          Sélectionnez l'instance {LAUNCHERS_UI[selectedLauncher]?.shortName || selectedLauncher}
        </h3>
        ...
        <div className="grid grid-cols-1 gap-3 max-h-60 overflow-y-auto pr-2 mb-6">
          {profiles.map((p: DetectedProfile) => (
            ...
            >
              <div>
                <p className="text-white font-bold">{p.label.replace(selectedLauncher + " : ", "")}</p>
                <p className="text-xs text-[#8888a0] truncate w-64">{p.options_path}</p>
              </div>
              {selectedProfileId === p.id && <CheckCircle2 className="text-white w-5 h-5" />}
            </div>
          ))}
        </div>
  ...
  return (
    <div className="p-6 bg-[#111111] h-full flex flex-col justify-center">
      <div className="text-center mb-8 mt-4">
        <h3 className="text-2xl text-white font-bold mb-2">Depuis d'autres launchers</h3>
        <p className="text-[#8888a0] max-w-sm mx-auto">
          Vous voulez changer ? Migrez vos profils existants en quelques secondes !
        </p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pb-8">
        {availableLaunchers.map((launcher) => {
          const ui = LAUNCHERS_UI[launcher] || { color: "bg-[#111111] border-[#2a2a2c]", shortName: launcher };
          return (
            <button
              key={launcher}
