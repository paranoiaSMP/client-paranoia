    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {profiles.length > 0 ? (
          ...
          <div className="md:col-span-2 lg:col-span-4 bg-[#1c1c1e]/50 border border-[#2a2a2c] border-dashed p-8 rounded-xl flex flex-col items-center justify-center text-center">
            <p className="text-[#8888a0] mb-4">{t("home.no_profiles")}</p>
            <button 
              onClick={() => setActiveTab("profils")}
              className="px-6 py-2 bg-white text-black font-bold rounded-lg shadow-[0_0_15px_rgba(255,255,255,0.2)]"
            >
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-6">
          {/* Big News */}
          <div className="bg-[#1c1c1e]/90 backdrop-blur-md border border-[#2a2a2c] rounded-xl overflow-hidden group cursor-pointer relative shadow-[inset_0_2px_15px_rgba(0,0,0,0.5)]">
            <div className="h-64 bg-[#111111] relative flex items-center justify-center overflow-hidden">
               <div className="absolute inset-0 bg-gradient-to-br from-accent-purple/20 to-black/80"></div>
               <Pickaxe className="w-16 h-16 text-accent-purple/30 z-10 drop-shadow-2xl" />
            </div>
            <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black to-transparent">
              <div className="mb-2">
                <span className="px-3 py-1 bg-accent-red text-white text-[10px] font-bold uppercase rounded shadow-lg">{t("home.news_badge")}</span>
              </div>
              <h3 className="text-2xl font-black text-white mb-1 group-hover:text-accent-purple transition-colors">{news[0]?.title || "Saison 3 : L'Éveil"}</h3>
              <p className="text-[#8888a0] text-sm line-clamp-2">{news[0]?.excerpt || "La nouvelle saison arrive avec son lot de nouveautés."}</p>
            </div>
          </div>
        </div>
      </div>
