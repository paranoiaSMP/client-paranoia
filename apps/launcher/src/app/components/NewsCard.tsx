      ...
      >
        ...
        {latest ? (
          <div className="mt-3 min-h-0 flex-1 flex flex-col">
            <motion.h3 layoutId={`title-${latest.title}-${id}`} className="line-clamp-2 text-sm font-semibold leading-snug text-white">
              {latest.title}
            </motion.h3>
            <motion.p layoutId={`description-${latest.title}-${id}`} className="mt-1.5 line-clamp-4 text-[11px] leading-snug text-[#a1a1aa]">
              {latest.excerpt}
            </motion.p>
          </div>
        ) : (
