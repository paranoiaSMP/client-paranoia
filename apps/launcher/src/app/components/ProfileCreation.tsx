...
export function ProfileCreation({
  step, setStep, connected, error,
  profileName, setProfileName,
  minecraftVersion, setMinecraftVersion, config,
  importSettings, setImportSettings,
  keybindSource, setKeybindSource, detectedProfiles,
  importOptions, setImportOptions,
  profileType, setProfileType,
  graphicsMode, setGraphicsMode,
  selectedType, selectedGraphics,
  handleInstall, installState
}: ProfileCreationProps) {
  const { t } = useTranslation();

  return (
    <div className="bg-[#1c1c1e] border-2 border-[#2a2a2c] rounded-xl p-8 max-w-2xl mx-auto shadow-2xl">
      <h3 className="text-2xl font-black text-white mb-6">{t("wizard.title")}</h3>
      
      {/* Step indicators */}
      <div className="flex gap-2 mb-8">
        {[1,2,3,4,5].map((s) => (
          <div key={s} className={`flex-1 h-3 rounded-sm transition-all ${step >= s ? 'bg-accent-purple shadow-[0_0_10px_rgba(157,13,242,0.5)]' : 'bg-[#2a2a2c]'}`}></div>
        ))}
      </div>
      
      {error && <div className="p-4 bg-red-900/50 border-2 border-red-500 rounded text-red-200 font-bold mb-6">{error}</div>}
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          {connected ? (
            ...
            <p className="text-accent-red font-bold">{t("wizard.step1_error")}</p>
          )}
        </div>
      )}
      ...
      {step === 2 && (
        <div className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-bold text-[#8888a0] mb-2">{t("wizard.name_label")}</label>
            <input 
              type="text" 
              value={profileName} 
          ...
          <div>
            <label className="block text-sm font-bold text-[#8888a0] mb-2">{t("wizard.version_label")}</label>
            <select 
            ...
            >
              {config?.supportedMinecraftVersions.map((v) => <option key={v} value={v}>{v}</option>)}
            </select>
          </div>
          
          <div className="pt-2">
            <label className="flex items-center gap-3 cursor-pointer group">
              <div className={`w-6 h-6 rounded border-2 flex items-center justify-center transition-colors ${importSettings ? 'bg-accent-purple border-accent-purple' : 'bg-[#0d0d0f] border-[#2a2a2c] group-hover:border-[#4a4a4c]'}`}>
                {importSettings && <CheckCircle2 className="w-4 h-4 text-white" strokeWidth={3} />}
              </div>
              <input type="checkbox" className="hidden" checked={importSettings} onChange={(e) => setImportSettings(e.target.checked)} />
              <div>
                <span className="font-bold text-white block">{t("wizard.import_settings")}</span>
                <span className="text-xs text-[#8888a0]">{t("wizard.import_settings_desc")}</span>
              </div>
            </label>
            ...
            {importSettings && (
              <div className="mt-4 pl-9 space-y-4 animate-in slide-in-from-top-2 duration-200">
                <div>
                  <label className="block text-xs font-bold text-[#8888a0] mb-2 uppercase tracking-wider">{t("wizard.which_profile")}</label>
                  <select 
                    value={keybindSource}
                  ...
                  >
                    ...
                    <optgroup label={t("wizard.detected")}>
                      {detectedProfiles.length === 0 && <option disabled>{t("wizard.searching")}</option>}
                ...
                <div className="space-y-3 pt-1">
                  <label className="flex items-center gap-3 cursor-pointer group">
                  <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${importOptions.keybinds ? 'bg-accent-purple border-accent-purple' : 'bg-[#0d0d0f] border-[#2a2a2c] group-hover:border-[#4a4a4c]'}`}>
                    {importOptions.keybinds && <CheckCircle2 className="w-3 h-3 text-white" strokeWidth={3} />}
                  </div>
                  <input type="checkbox" className="hidden" checked={importOptions.keybinds} onChange={(e) => setImportOptions({...importOptions, keybinds: e.target.checked})} />
                  <span className="text-sm font-bold text-white">{t("wizard.keybinds")}</span>
                </label>

                <label className="flex items-center gap-3 cursor-pointer group">
                  <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${importOptions.sensitivity ? 'bg-accent-purple border-accent-purple' : 'bg-[#0d0d0f] border-[#2a2a2c] group-hover:border-[#4a4a4c]'}`}>
                    {importOptions.sensitivity && <CheckCircle2 className="w-3 h-3 text-white" strokeWidth={3} />}
                  </div>
                  <input type="checkbox" className="hidden" checked={importOptions.sensitivity} onChange={(e) => setImportOptions({...importOptions, sensitivity: e.target.checked})} />
                  <span className="text-sm font-bold text-white">{t("wizard.sensitivity")}</span>
                </label>

                <label className="flex items-center gap-3 cursor-pointer group">
                  <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${importOptions.graphics ? 'bg-accent-purple border-accent-purple' : 'bg-[#0d0d0f] border-[#2a2a2c] group-hover:border-[#4a4a4c]'}`}>
                    {importOptions.graphics && <CheckCircle2 className="w-3 h-3 text-white" strokeWidth={3} />}
                  </div>
                  <input type="checkbox" className="hidden" checked={importOptions.graphics} onChange={(e) => setImportOptions({...importOptions, graphics: e.target.checked})} />
                  <span className="text-sm font-bold text-white">{t("wizard.graphics")}</span>
                </label>
              </div>
            </div>
            )}
          ...
          <div className="grid gap-3">
            {config?.profileTypes.map((t) => (
              ...
              >
                ...
                <div className="text-sm font-medium text-[#8888a0]">{t.description}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid gap-3">
            {config?.graphicsModes.map((g) => (
              ...
              >
                ...
                <div className="text-sm font-medium text-[#8888a0]">{g.description}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 bg-[#0d0d0f] rounded border-2 border-[#2a2a2c] space-y-3 font-medium">
            ...
            <div className="flex justify-between border-b border-[#2a2a2c] py-3">
              ...
              <span className="font-black text-accent-purple">{selectedType?.label}</span>
            </div>
            <div className="flex justify-between pt-3">
              <span className="text-[#8888a0]">{t("wizard.graphics_label")}</span>
              <span className="font-black text-white">{selectedGraphics?.label}</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex justify-between mt-8 pt-6 border-t border-[#2a2a2c]">
        ...
        {step < 5 ? (
          ...
          >
            {installState === 'running' ? t("wizard.preparing") : t("wizard.forge")}
          </button>
        )}
      </div>
    </div>
