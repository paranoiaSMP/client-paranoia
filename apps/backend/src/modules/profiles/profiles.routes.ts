 ...
 */
async function prepareInstance(profile: {
	id: string;
	minecraftVersion: string;
}): Promise<void> {
	await ensureInstanceLayout(profile.id);

	void ensureFabricApi(
		...
		.catch((err) => {
			...
			console.warn(
				`[Profils] Fabric API non installe pour ${profile.id}:`,
				err,
			);
		});
}
...
profilesRouter.post("/migrate", async (req, res, next) => {
	try {
		...
		if (!optionsPath) {
			return res
				.status(400)
				.json({ message: "optionsTxtPath is required for migration" });
		}
